		...
		function Game(){
			...
			function Snack(Class, Value, MaxTTL) {
				this.Class = Class;
				this.Value = Value;
				this.MaxTTL = MaxTTL;
			};			
			...
			snacks[0] = new Snack('red', 10, 1.2);
			snacks[1] = new Snack('cookie', 20, 1);
			snacks[2] = new Snack('green', 50, 0.75);
			...
			function Click(event)
			{
				...
				if(target.className.indexOf('snack') > -1){
			...
			function Remove(id) {
				var snack = game.querySelector("#" + id);
			...
			function Spawn() {
				...
				var ttl = Math.floor( ( Math.random() * parseInt(snack.MaxTTL) * 1000 ) + 1000 ); //between 1s and MaxTTL				
				var x = Math.floor( ( Math.random() * ( game.offsetWidth - 40 ) ) );
				var y = Math.floor( ( Math.random() * ( game.offsetHeight -  44 ) ) );
				
				var fragment = document.createElement('span');
				fragment.id = "snack-" + id;
				fragment.setAttribute('class', "snack " + snack.Class);
				fragment.setAttribute('data-value', snack.Value);		
			...
			this.Start = function() {
				...
				function timer()
				{
					...
					if (count <= 0)
					{
						var left = document.querySelectorAll("section#game .snack");
